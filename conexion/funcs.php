<!--?php
	...
	function isNull($nombre, $user, $pass, $pass_con, $email,$tipo_usuario, $red){
		if(strlen(trim($nombre)) < 1 || strlen(trim($user)) < 1 || strlen(trim($pass)) < 1 || strlen(trim($pass_con)) < 1 || strlen(trim($email)) < 1 || strlen(trim($tipo_usuario)) < 1 || strlen(trim($red)) < 1)
		{
			return true;
			} else {
			return false;
		...
		if($rows > 0) {
			...
			if(isActivo($usuario)){
				...
				if($validaPassw){
					...
					$_SESSION['tipo_usuario'] = $id_tipo; //acabo de cambiar de tipo_usuario//
